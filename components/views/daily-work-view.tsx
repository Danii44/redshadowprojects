...
export function DailyWorkView({
  ...
  // Active tasks assigned to me (or all active tasks if Admin/Leader)
    ...
    const parsed: LogEntry[] = (data ?? [])
      .map((item: any) => {
        ...
        return {
          ...
          project_name: projectMap.get(item.tasks?.project_id) || "Project",
          notes: notesText,
          completionPct: comp,
          hoursSpent: hours,
          blocker: block,
  ...
  return (
    <div className="space-y-7">
        ...
        <div>
          ...
          <p className="mt-1 text-xs font-bold text-slate-500">
            {role === "Team Member"
              ...
              : "Assign daily tasks to team members and track team progress and standups."}
          </p>
        </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between border-b border-slate-200 pb-3">
          <h2 className="text-lg font-black text-slate-900">
            {role === "Team Member" ? "Your Daily Tasks Today" : "Active Tasks Assigned To Team"}
          </h2>
          <span className="text-xs font-bold text-slate-500">
            {myAssignedTasks.length} active tasks
          </span>
        </div>
              ...
              >
                <div>
                  ...
                  <p className="mt-1 text-xs font-semibold text-slate-500">
                    Assigned to: {task.owner} · Target: {task.due}
                  </p>
          ...
          {!myAssignedTasks.length && (
            <div className="col-span-full rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center">
              ...
              <h3 className="font-black text-slate-800 text-base">
                No active tasks listed for today!
              </h3>
              <p className="mt-1 text-xs font-semibold text-slate-400">
                {role === "Team Member"
                  ? "Admin or Project Leader hasn't assigned a task yet. Click '+ Add What I'm Working On Today' to add your task."
                  : "Assign daily tasks to team members using the button above."}
              </p>
              <button
                onClick={() => setAddingSelfTask(true)}
                className="mt-4 inline-flex items-center gap-2 rounded-xl bg-[#e3292f] px-4 py-2.5 text-xs font-black text-white hover:bg-red-700 transition shadow-xs cursor-pointer"
              >
                <Plus size={15} />
                + Add What I'm Working On Today
              </button>
      ...
      {/* DAILY PROGRESS LOG HISTORY STREAM */}
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <div>
            <h3 className="text-base font-black text-slate-900">
              {role === "Team Member"
                ? "Your Recent Progress Logs"
                : "Team Daily Work Stream"}
            </h3>
            <p className="text-xs font-semibold text-slate-400">
              Activity entries and roadblocks recorded by team members
            </p>
          </div>
            ...
            <form onSubmit={handleAddSelfTask} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 border-t border-slate-100 pt-3">
                <button
                  type="button"
                  onClick={() => setAddingSelfTask(false)}
                  className="h-10 rounded-xl border border-slate-200 px-4 text-xs font-bold text-slate-700 hover:bg-slate-50"
                >
                  Cancel
                </button>
                <button
