...
export async function POST(request: Request) {
  ...
  if (!name || !email || !role || !["admin", "project_leader", "team_member"].includes(role)) {
  ...
  const profileWrite = existingProfile
    ? adminClient
        .from("users")
        .update({ auth_user_id: data.user.id, name, role, active: true, updated_at: new Date().toISOString() })
  ...
  if (profileError) {
    ...
    return NextResponse.json({ error: `User was not linked to a company profile: ${profileError.message}` }, { status: 500 });
  }
